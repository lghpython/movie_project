...
def admin_auth(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        auths = admin.role.auths  # 取出当前管理员的角色类型地址
        auths = list(map(lambda v: int(v), auths.split(',')))  # 字符串转换列表
        auth_list = Auth.query.all()  # 取出所有权限数据
        # 匹配角色权限，生成角色权限列表
        urls = [v.url for v in auth_list for val in auths if val == v.id]
        rule = request.url_rule  # 获取路由地址
        if str(rule) not in urls:
            # 拦截没获取权限的路由地址，跳转404页面
            abort(404)
        return f(*args, **kwargs)
...
def tag_edit(id=None):
    ...
    if form.validate_on_submit():
        ...
        if tag.name != data["name"] and tag_count == 1:
            flash("标签已经存在!", "err")
            return redirect(url_for("admin.tag_edit", tag=tag))
        tag.name = data["name"]
        db.session.add(tag)
        db.session.commit()
        flash("标签添加成功!", "ok")
...
def movie_del(id=None):
    data = Movie.query.get_or_404(int(id))
...
def role_edit(id=None):
    ...
    auths = role.auths
    if request.method == "GET" and auths != '':
        form.auths.data = list(map(lambda v: int(v), auths.split(','))) or ''
